...
test.describe('Complete E2E Workflow with Azure OpenAI', () => {
  ...
  test('full user journey: campaign → character → portrait → DM chat', async ({ page }) => {
    ...
    await expect(page.locator('h1').first()).toContainText('Securing the Realm', { timeout: 10000 });
    ...
    const createCampaignBtn = page.locator('button:has-text("Create Campaign"), button:has-text("Create"), button:has-text("New Campaign")').first();
    ...
    // Submit campaign
    const submitCampaignBtn = page.locator('button[type="submit"], button:has-text("Create Campaign"), button:has-text("Create")').first();
    ...
    // Fill character name
    const characterNameInput = page.locator('input[name="name"], input[placeholder*="character" i], input[placeholder*="name" i]').first();
    ...
    const backstoryInput = page.locator('textarea[name="backstory"], input[name="backstory"]').first();
    if (await backstoryInput.count() > 0) {
      await backstoryInput.fill('A skilled ranger from the Moonwood, trained in tracking and survival. Seeks to protect the forest realm from encroaching darkness.');
    ...
    const submitCharacterBtn = page.locator('button[type="submit"], button:has-text("Create Character"), button:has-text("Create")').first();
    ...
    const portraitBtn = page.locator('button:has-text("Character Portrait"), button:has-text("Portrait"), button:has-text("Generate Portrait")').first();
    ...
    if (await portraitBtn.count() > 0) {
      ...
      const imageDisplay = page.locator('img[alt*="Character" i], img[alt*="Portrait" i], .image-display img').first();
      if (await imageDisplay.count() > 0) {
        const imageSrc = await imageDisplay.getAttribute('src');
        console.log(`✅ Portrait generated successfully: ${imageSrc?.substring(0, 50)}...`);
        
        // Verify image is not broken
        const imageNaturalWidth = await imageDisplay.evaluate((img: HTMLImageElement) => img.naturalWidth);
        if (imageNaturalWidth > 0) {
          console.log(`✅ Portrait image loaded successfully (width: ${imageNaturalWidth}px)`);
        } else {
      ...
      console.warn('⚠️ Portrait generation button not found - skipping portrait step');
    }
    ...
    console.log('✅ Entered chat message: "I look around carefully. What do I see?"');
    ...
    if (messageCount > 0) {
      ...
      console.log(`✅ Last message preview: ${messageText?.substring(0, 100)}...`);
    }
    ...
    for (let i = 0; i < Math.min(imageCount, 5); i++) {
      ...
      console.log(`Image ${i}: alt="${alt}", natural=${naturalWidth}px, displayed=${clientWidth}px`);
      
      // Check if image is truncated
      if (naturalWidth > 0 && clientWidth < naturalWidth * 0.5) {
        console.warn(`⚠️ Image ${i} may be truncated (showing ${Math.round(clientWidth/naturalWidth*100)}% of original)`);
      }
    }
