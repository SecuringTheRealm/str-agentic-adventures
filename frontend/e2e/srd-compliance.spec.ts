...
test.describe('D&D 5e SRD Compliance Tests', () => {
      ...
      if (await createCharacterBtn.count() > 0) {
        ...
        if (await raceSelect.count() > 0) {
          ...
          for (const option of options) {
            if (option && option.trim() && option !== 'Select Race') {
  ...
  test('should only allow SRD-compliant character classes', async ({ page }) => {
    ...
    if (await createCampaignBtn.count() > 0) {
      ...
      if (await createCharacterBtn.count() > 0) {
        ...
        if (await classSelect.count() > 0) {
          ...
          for (const option of options) {
            if (option && option.trim() && option !== 'Select Class') {
    ...
    if (await createCampaignBtn.count() > 0) {
      ...
      if (await createCharacterBtn.count() > 0) {
        ...
        for (const ability of abilities) {
          const abilityInput = page.locator(`input[name="abilities.${ability}"], input[data-ability="${ability}"]`);
  ...
  test('should provide standard D&D 5e dice rolling', async ({ page }) => {
    ...
    const diceInterface = page.locator('[data-testid="dice-roller"], .dice-roller');
    ...
    if (await diceInterface.count() > 0) {
      ...
      for (const die of standardDice) {
        ...
        if (await diceButton.count() > 0) {
          ...
          const result = page.locator('[data-testid="dice-result"], .dice-result');
          if (await result.count() > 0) {
            const resultText = await result.textContent();
            if (resultText) {
              const rollValue = parseInt(resultText.match(/\d+/)?.[0] || '0');
  ...
  test('should support D&D 5e level progression rules', async ({ page }) => {
    ...
    if (await characterSheet.count() > 0) {
      ...
      const levelDisplay = page.locator('text="Level", [data-testid="character-level"]');
      
      if (await levelDisplay.count() > 0) {
        // Characters should start at level 1
        await expect(page.locator('text="Level 1", text="1st Level"')).toBeVisible();
      }
      
      // Look for experience points
      const xpDisplay = page.locator('text="Experience", text="XP", [data-testid="experience"]');
      ...
      const proficiencyDisplay = page.locator('text="Proficiency", [data-testid="proficiency-bonus"]');
  ...
  test('should implement D&D 5e combat mechanics', async ({ page }) => {
    ...
    const combatInterface = page.locator('[data-testid="combat-interface"], .combat-interface');
    
    if (await combatInterface.count() > 0) {
      // Should have initiative system
      const initiative = page.locator('text="Initiative", [data-testid="initiative"]');
      if (await initiative.count() > 0) {
        await expect(initiative).toBeVisible();
      }
      
      // Should have standard D&D actions
      const standardActions = ['Attack', 'Move', 'Dash', 'Dodge', 'Help', 'Hide', 'Ready', 'Search'];
      ...
      const hpDisplay = page.locator('text="Hit Points", text="HP", [data-testid="hit-points"]');
      const acDisplay = page.locator('text="Armor Class", text="AC", [data-testid="armor-class"]');
  ...
  test('should provide spell system for spellcasting classes', async ({ page }) => {
    ...
    if (await createCampaignBtn.count() > 0) {
      ...
      if (await createCharacterBtn.count() > 0) {
        ...
        const spellInterface = page.locator('[data-testid="spells"], .spells, text="Spells"');
        
        if (await spellInterface.count() > 0) {
          // Should show spell slots
          const spellSlots = page.locator('text="Spell Slots", [data-testid="spell-slots"]');
          if (await spellSlots.count() > 0) {
            await expect(spellSlots).toBeVisible();
          }
          
          // Should show cantrips for level 1 wizard
          const cantrips = page.locator('text="Cantrips", [data-testid="cantrips"]');
