...
test.describe('Complete User Journey - Manual Testing as a User', () => {
  test('complete game flow: campaign creation to character creation to gameplay', async ({ page }) => {
    ...
    await page.screenshot({ path: 'screenshots/journey/01-app-loaded.png', fullPage: true });
    ...
    const campaignButtons = page.locator('button:has-text("Create"), button:has-text("Campaign"), button:has-text("New")');
    ...
    if (campaignCount > 0) {
      ...
      await page.screenshot({ path: 'screenshots/journey/02-campaign-creation-clicked.png', fullPage: true });
      
      // Fill campaign details
      const nameField = page.locator('input[name="name"], input[placeholder*="name"], input[placeholder*="campaign"]');
      if (await nameField.count() > 0) {
        console.log('📝 Filling campaign name...');
        await nameField.fill('The Lost Mines of Phandelver');
        
        // Look for other campaign fields
        const settingField = page.locator('input[name="setting"], textarea[name="setting"], input[placeholder*="setting"]');
        if (await settingField.count() > 0) {
          console.log('🌍 Filling campaign setting...');
          await settingField.fill('The Sword Coast region of Faerûn, featuring the frontier town of Phandalin and the mysterious Lost Mine of Phandelver.');
        }
        ...
        const homebrewField = page.locator('textarea[name="homebrew_rules"], input[name="homebrew_rules"]');
        if (await homebrewField.count() > 0) {
          console.log('📜 Adding homebrew rules...');
          await homebrewField.fill('Allow players to use inspiration dice for critical hit confirmation');
        }
        
        await page.screenshot({ path: 'screenshots/journey/03-campaign-form-filled.png', fullPage: true });
        ...
        if (await submitButton.count() > 0) {
          ...
          await page.screenshot({ path: 'screenshots/journey/04-campaign-submitted.png', fullPage: true });
        }
      }
    }
    ...
    const characterHeader = page.locator('text="Choose Your Character", text="Character Selection", h2:has-text("Character")');
    if (await characterHeader.count() > 0) {
      console.log('✅ Character selection screen found');
      await expect(characterHeader).toBeVisible();
      
      await page.screenshot({ path: 'screenshots/journey/05-character-selection.png', fullPage: true });
      
      // Look for character creation options
      const createCharacterBtn = page.locator('button:has-text("Create Character"), button:has-text("Create New"), button:has-text("New Character")');
      const browseCharacterBtn = page.locator('button:has-text("Browse Characters"), button:has-text("Predefined"), button:has-text("Browse")');
      
      console.log(`🔍 Found ${await createCharacterBtn.count()} create character buttons`);
      console.log(`🔍 Found ${await browseCharacterBtn.count()} browse character buttons`);
      ...
      if (await createCharacterBtn.count() > 0) {
        ...
        await page.waitForTimeout(2000);
        await page.screenshot({ path: 'screenshots/journey/06-character-creation-form.png', fullPage: true });
        
        // Fill character creation form according to D&D 5e SRD
        const nameInput = page.locator('input[name="name"], input[placeholder*="name"], input[placeholder*="character"]');
        ...
        for (let i = 0; i < abilities.length; i++) {
          const abilityInput = page.locator(`input[name="abilities.${abilities[i]}"], input[data-ability="${abilities[i]}"]`);
        ...
        const backstoryField = page.locator('textarea[name="backstory"], input[name="backstory"]');
        if (await backstoryField.count() > 0) {
          console.log('📖 Writing character backstory...');
          await backstoryField.fill('A veteran warrior from the mountain halls of the Ironshield clan, seeking adventure and glory in the wider world.');
        }
        
        await page.screenshot({ path: 'screenshots/journey/07-character-form-completed.png', fullPage: true });
        ...
        if (await submitCharacterBtn.count() > 0) {
          ...
          await page.waitForTimeout(3000);
          await page.screenshot({ path: 'screenshots/journey/08-character-created.png', fullPage: true });
        }
        ...
        await page.waitForTimeout(2000);
        await page.screenshot({ path: 'screenshots/journey/06-predefined-characters.png', fullPage: true });
        
        // Select first available character
        const characterCard = page.locator('.character-card, [data-testid="character-option"], button:has-text("Select")');
        if (await characterCard.count() > 0) {
          console.log('👤 Selecting first available character...');
          await characterCard.first().click();
          
          await page.waitForTimeout(2000);
          await page.screenshot({ path: 'screenshots/journey/08-predefined-character-selected.png', fullPage: true });
        }
      }
    }
    ...
    const gameInterface = page.locator('[data-testid="game-interface"], .game-interface, text="Game Interface"');
    if (await gameInterface.count() > 0) {
      console.log('✅ Game interface found');
      await expect(gameInterface).toBeVisible();
      
      await page.screenshot({ path: 'screenshots/journey/09-game-interface.png', fullPage: true });
      ...
      if (await chatInput.count() > 0 && await sendButton.count() > 0) {
        ...
        await chatInput.fill('I look around the tavern, taking note of the other patrons and searching for any potential allies or threats.');
        await page.screenshot({ path: 'screenshots/journey/10-chat-input.png', fullPage: true });
        
        await sendButton.click();
        console.log('📤 Sent message to AI DM');
        
        await page.waitForTimeout(2000);
        await page.screenshot({ path: 'screenshots/journey/11-chat-sent.png', fullPage: true });
        
        // Test skill check request
        await page.waitForTimeout(1000);
        await chatInput.fill('I want to make a Perception check to notice any hidden details in the room.');
        await sendButton.click();
        console.log('🎲 Requested Perception check');
        
        await page.waitForTimeout(2000);
        await page.screenshot({ path: 'screenshots/journey/12-skill-check.png', fullPage: true });
      }
      
      // Explore dice rolling
      const diceRoller = page.locator('[data-testid="dice-roller"], .dice-roller, button:has-text("d20")');
      if (await diceRoller.count() > 0) {
        ...
        if (await d20Button.count() > 0) {
          ...
          await page.waitForTimeout(1000);
          await page.screenshot({ path: 'screenshots/journey/13-d20-roll.png', fullPage: true });
        }
        ...
        await page.screenshot({ path: 'screenshots/journey/14-dice-testing.png', fullPage: true });
      }
      
      // Explore character sheet
      const characterSheet = page.locator('[data-testid="character-sheet"], .character-sheet, text="Character Sheet"');
      if (await characterSheet.count() > 0) {
        ...
        const d5eElements = [
          'text="Strength"', 'text="Dexterity"', 'text="Constitution"',
          'text="Intelligence"', 'text="Wisdom"', 'text="Charisma"',
          'text="Hit Points"', 'text="Armor Class"', 'text="Proficiency"'
        ];
        ...
        await page.screenshot({ path: 'screenshots/journey/15-character-sheet.png', fullPage: true });
      }
      
      // Test navigation
      const backButton = page.locator('button:has-text("Back to Campaigns"), button:has-text("← Back")');
      if (await backButton.count() > 0) {
        console.log('🔙 Testing navigation back to campaigns...');
        await backButton.click();
        
        await page.waitForTimeout(2000);
        await page.screenshot({ path: 'screenshots/journey/16-back-to-campaigns.png', fullPage: true });
        
        // Verify we're back at campaign selection
        const campaignInterface = page.locator('.campaign-setup, .campaign-selection, text="Campaign"');
    ...
    await page.screenshot({ path: 'screenshots/journey/17-test-complete.png', fullPage: true });
