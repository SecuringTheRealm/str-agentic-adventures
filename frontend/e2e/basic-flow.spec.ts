...
test.describe('Securing the Realm - Basic Application Flow', () => {
  ...
  test('should display campaign selection by default', async ({ page }) => {
    ...
    await expect(page.locator('.campaign-setup, .campaign-selection, [data-testid="campaign-hub"]')).toBeVisible();
    
    await page.screenshot({ path: 'screenshots/campaign-selection.png', fullPage: true });
  });
});