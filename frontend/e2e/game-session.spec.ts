...
test.describe('Game Session Flow', () => {
  ...
  test('should start a game session after campaign and character creation', async ({ page }) => {
    ...
    if (await createCampaignBtn.count() > 0) {
      ...
      const settingField = page.locator('input[name="setting"], textarea[name="setting"]');
      if (await settingField.count() > 0) {
        await settingField.fill('A mysterious realm where ancient magic still flows');
      }
      ...
      const createCharacterBtn = page.locator('button:has-text("Create Character")');
      ...
      await expect(page.locator('[data-testid="game-interface"], .game-interface')).toBeVisible({ timeout: 15000 });
      
      await page.screenshot({ path: 'screenshots/game-session-started.png', fullPage: true });
    }
  });
  ...
  test('should display game interface components', async ({ page }) => {
    ...
    if (await gameInterface.count() === 0) {
      ...
      if (await createBtn.count() > 0) {
        ...
        const quickCharacterBtn = page.locator('button:has-text("Browse Characters"), .character-card').first();
    ...
    await page.screenshot({ path: 'screenshots/game-interface-components.png', fullPage: true });
  });
  ...
  test('should allow interaction with AI Dungeon Master', async ({ page }) => {
    ...
    const chatInput = page.locator('[data-testid="chat-input"], input[placeholder*="message"], input[placeholder*="action"], textarea[placeholder*="message"]');
    ...
    if (await chatInput.count() > 0 && await sendButton.count() > 0) {
      ...
      await page.screenshot({ path: 'screenshots/chat-interaction.png', fullPage: true });
    }
  });
  ...
  test('should provide dice rolling functionality', async ({ page }) => {
    ...
    const diceRoller = page.locator('[data-testid="dice-roller"], .dice-roller, button:has-text("d20"), button:has-text("Roll")');
    ...
    if (await diceRoller.count() > 0) {
      ...
      for (const diceType of diceTypes) {
        ...
        if (await diceButton.count() > 0) {
          ...
          await expect(page.locator('[data-testid="dice-result"], .dice-result, .roll-result')).toBeVisible();
          
          await page.screenshot({ path: `screenshots/dice-roll-${diceType}.png` });
        }
      }
    }
  });
  ...
  test('should display character sheet information', async ({ page }) => {
    ...
    const characterSheet = page.locator('[data-testid="character-sheet"], .character-sheet');
    ...
    if (await characterSheet.count() > 0) {
      ...
      await page.screenshot({ path: 'screenshots/character-sheet.png', fullPage: true });
    }
  });
  ...
  test('should handle combat encounters', async ({ page }) => {
    ...
    const combatElements = page.locator('[data-testid="battle-map"], .battle-map, [data-testid="combat-interface"], .combat-interface');
    ...
    if (await combatElements.count() > 0) {
      ...
      const initiative = page.locator('[data-testid="initiative"], .initiative, text="Initiative"');
      if (await initiative.count() > 0) {
        await expect(initiative).toBeVisible();
      }
      
      // Look for action buttons
      const actionButtons = page.locator('button:has-text("Attack"), button:has-text("Move"), button:has-text("Cast Spell")');
      if (await actionButtons.count() > 0) {
        await expect(actionButtons.first()).toBeVisible();
      }
      
      await page.screenshot({ path: 'screenshots/combat-interface.png', fullPage: true });
    }
  });
  ...
  test('should allow saving and loading game sessions', async ({ page }) => {
    ...
    const saveButton = page.locator('button:has-text("Save"), [data-testid="save-game"]');
    const loadButton = page.locator('button:has-text("Load"), [data-testid="load-game"]');
  ...
  test('should have back navigation to campaigns', async ({ page }) => {
    ...
    if (await backButton.count() > 0) {
      ...
      await expect(page.locator('.campaign-setup, .campaign-selection')).toBeVisible({ timeout: 10000 });
      
      await page.screenshot({ path: 'screenshots/back-to-campaigns.png', fullPage: true });
    }
  });
});