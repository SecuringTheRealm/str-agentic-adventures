...
export const testApiConnectivity = async (baseUrl: string): Promise<boolean> => {
  try {
    ...
    const templatesResponse = await fetch(`${baseUrl}/api/game/campaign/templates`, {
      method: 'GET',
      headers: {
        'Content-Type': 'application/json',
      },
    ...
    if (!templatesResponse.ok) {
      console.error('❌ Templates endpoint failed:', templatesResponse.status, templatesResponse.statusText);
      return false;
    }
    
    const data = await templatesResponse.json();
    console.log('✅ Templates endpoint working, templates count:', data.templates?.length || 0);
    return true;
...
export const validateApiUrl = (url: string): { isValid: boolean; issues: string[] } => {
  const issues: string[] = [];
