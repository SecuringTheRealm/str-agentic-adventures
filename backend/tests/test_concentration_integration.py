...
class TestConcentrationSpellIntegration:
    ...
    @pytest.mark.asyncio
    async def test_concentration_spell_starts_concentration(self, concentration_spell_data, cast_spell_request):
        """Test that casting a concentration spell starts concentration."""
        with patch('app.api.game_routes._get_spell_data') as mock_get_spell, \
             patch('app.api.game_routes._calculate_spell_effects') as mock_calc_effects, \
             patch('app.plugins.rules_engine_plugin.RulesEnginePlugin') as mock_rules_engine:
    ...
    @pytest.mark.asyncio
    async def test_non_concentration_spell_no_concentration_started(self, non_concentration_spell_data, cast_spell_request):
    ...
    @pytest.mark.asyncio
    async def test_concentration_spell_breaks_existing_concentration(self, concentration_spell_data, cast_spell_request):
        ...
        with patch('app.api.game_routes._get_spell_data') as mock_get_spell, \
            ...
            # Verify response indicates success
            assert response.success is True
            assert response.concentration_broken is False  # Should be False because we successfully started new concentration


    @pytest.mark.asyncio
    async def test_concentration_spell_failure_doesnt_break_cast(self, concentration_spell_data, cast_spell_request):
