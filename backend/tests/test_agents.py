...
class TestAgentMocking:
    ...
    def test_agent_interface_contracts(self):
        ...
        mock_scribe = Mock()
        mock_scribe.create_character = AsyncMock(return_value={
            "id": "test_char_123",
            "name": "Test Character", 
            "class": "fighter",
            "race": "human",
    ...
    def test_dungeon_master_interface(self):
        ...
        mock_dm = Mock()
        mock_dm.create_campaign = AsyncMock(return_value={
            "id": "camp_123",
            "name": "Test Campaign",
            "setting": "fantasy"
        })
        mock_dm.process_input = AsyncMock(return_value={
    ...
    def test_artist_agent_interface(self):
        ...
        mock_artist = Mock()
        mock_artist.generate_character_portrait = AsyncMock(return_value={
            "image_url": "http://test.com/portrait.jpg"
        })
        mock_artist.illustrate_scene = AsyncMock(return_value={
            "image_url": "http://test.com/scene.jpg"
        })
        mock_artist.create_item_visualization = AsyncMock(return_value={
            "image_url": "http://test.com/item.jpg"
        })
