...
def test_campaign_endpoint_with_missing_config():
    ...
    try:
        ...
        assert "Azure OpenAI configuration" in detail, f"Error message should mention Azure OpenAI configuration: {detail}"
        assert "agentic demo requires" in detail, f"Error should mention this is an agentic demo: {detail}"
        assert "AZURE_OPENAI_ENDPOINT" in detail, f"Error should list required env vars: {detail}"
        
        print("✓ Test passed: Campaign endpoint properly handles missing Azure OpenAI configuration")
        return True
    ...
    try:
        ...
        if response.status_code == 503:
            ...
            if "Azure OpenAI configuration" in detail:
                print("✗ Test failed: Still getting configuration error even with config set")
                return False
        
        print(f"✓ Test passed: With configuration, got status {response.status_code} (not 503 config error)")
        return True
