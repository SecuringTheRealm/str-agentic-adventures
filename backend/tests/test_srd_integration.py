...
class TestSRDIntegration:
    ...
    async def test_complete_character_creation_with_srd_features(self, scribe_agent):
        ...
        with patch('app.agents.scribe_agent.get_session') as mock_session:
            ...
            print(f"HP: {result['hit_points']['current']}/{result['hit_points']['maximum']}")
            print(f"Abilities: STR {abilities['strength']}, DEX {abilities['dexterity']}, CON {abilities['constitution']}")
