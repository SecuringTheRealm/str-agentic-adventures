...
class Settings(BaseSettings):
    ...
    # Model Deployments
    azure_openai_chat_deployment: Optional[str] = os.getenv("AZURE_OPENAI_CHAT_DEPLOYMENT")
    azure_openai_embedding_deployment: Optional[str] = os.getenv("AZURE_OPENAI_EMBEDDING_DEPLOYMENT")
    azure_openai_image_deployment: Optional[str] = os.getenv("AZURE_OPENAI_IMAGE_DEPLOYMENT")

    # Semantic Kernel Settings
    semantic_kernel_debug: bool = os.getenv("SEMANTIC_KERNEL_DEBUG", "False").lower() == "true"

    # Storage Settings
    storage_connection_string: Optional[str] = os.getenv("STORAGE_CONNECTION_STRING")
