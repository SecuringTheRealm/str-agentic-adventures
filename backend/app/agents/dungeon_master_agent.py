...
class DungeonMasterAgent:
    ...
    def _register_plugins(self):
        ...
        try:
            ...
            self.memory_plugin = NarrativeMemoryPlugin()
            rules_engine = RulesEnginePlugin()

            # Register plugins with the kernel
            self.kernel.import_skill(self.memory_plugin, "Memory")
        ...
        try:
            ...
            if input_type == "narrative":
                ...
                if narrative_result.get("significant_moment", False):
                    scene_art = await artist.illustrate_scene(
                ...
                if context.get("combat_state") == "active":
                    ...
                    if "map_id" in context:
                        updated_map = (
                            await combat_cartographer.update_map_with_combat_state(
                                context["map_id"], combat_result.get("updates", {})
                            )
                        )
                ...
                recall = self.memory_plugin.recall_facts(
                    input_details.get("query", ""),
                    input_details.get("category", ""),
                )
                facts = recall.get("facts", [])
                if facts:
                    response["message"] = "\n".join(f["content"] for f in facts)
                else:
                    response["message"] = "No relevant facts found."
            ...
            # Record event in narrative memory
            self.memory_plugin.record_event(
                user_input,
                context.get("location", "unknown"),
                context.get("character_id", "player"),
            )
    ...
    async def _analyze_input(
        ...
        if any(
            term in input_lower
            for term in ["attack", "fight", "cast spell", "initiative", "roll"]
        ):
        ...
        if any(
            term in input_lower
            for term in ["inventory", "equip", "level up", "spell", "ability"]
        ):
            return "character", {"update_type": "inventory"}

        # Check for memory recall requests
        if any(term in input_lower for term in ["recall", "remember", "history"]):
            return "memory", {"query": user_input}
