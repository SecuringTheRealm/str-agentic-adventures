...
class NarratorAgent:
        ...
        try:
            ...
            if campaign_id and hasattr(self, 'narrative_generation'):
                ...
                if narrative_state.get("status") == "success":
                    ...
                    if active_arcs:
                        arguments["active_story_arcs"] = ", ".join([arc["title"] for arc in active_arcs])
            ...
            if hasattr(self, 'narrative_memory'):
                ...
                if location_memories.get("status") == "success" and location_memories["facts"]:
                    # Use memory to enrich location description
                    relevant_facts = [f["content"] for f in location_memories["facts"][:2]]
                    if relevant_facts:
    ...
    async def process_action(self, action: str, context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if hasattr(self, 'narrative_generation'):
                ...
                choice_type = "general"
                if any(word in action.lower() for word in ["fight", "attack", "combat"]):
                    choice_type = "combat"
                elif any(word in action.lower() for word in ["talk", "speak", "persuade", "negotiate"]):
                    choice_type = "social"
                elif any(word in action.lower() for word in ["explore", "investigate", "search"]):
                ...
                if advance_result.get("status") == "success":
                    ...
                    if activated_points:
                        description += f" Your action triggers significant developments in the story."
                        consequences["plot_points_activated"] = [p["title"] for p in activated_points]
                    
                    completed_points = advance_result.get("completed_plot_points", [])
                    if completed_points:
                        description += f" You have successfully resolved important story elements."
                        consequences["plot_points_completed"] = [p["title"] for p in completed_points]
            ...
            if hasattr(self, 'narrative_memory') and campaign_id:
                ...
                if any(word in action.lower() for word in ["help", "save", "protect", "sacrifice"]):
    ...
    async def create_campaign_story(self, campaign_context: Dict[str, Any]) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if hasattr(self, 'narrative_generation'):
                ...
                main_arc_result = self.narrative_generation.create_story_arc(
                    ...
                    character_ids=",".join(characters) if isinstance(characters, list) else str(characters)
                )
                ...
                if main_arc_result.get("status") == "success":
                    ...
                    if narrative_state.get("status") == "not_found":
                        ...
                        new_state.active_story_arcs.append(main_arc_result["story_arc_id"])
                        self.narrative_generation.narrative_states[campaign_id] = new_state
                    else:
                        # Add to existing state
                        if campaign_id in self.narrative_generation.narrative_states:
                            self.narrative_generation.narrative_states[campaign_id].active_story_arcs.append(
            ...
            if hasattr(self, 'narrative_memory'):
                ...
                self.narrative_memory.record_event(
                    ...
                    setting,
                    ",".join(characters) if isinstance(characters, list) else str(characters),
                    9
                )
