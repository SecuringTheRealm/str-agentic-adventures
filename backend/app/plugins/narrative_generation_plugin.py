...
class NarrativeGenerationPlugin:
    ...
    def _initialize_choice_templates(self) -> Dict[str, Dict[str, Any]]:
        ...
        return {
            "exploration": {
                "investigate": {"text": "Investigate further", "consequences": {"info_gained": True}},
                "proceed": {"text": "Proceed cautiously", "consequences": {"safety": True}},
                "retreat": {"text": "Retreat and regroup", "consequences": {"resources_preserved": True}}
            },
            "social": {
                "persuade": {"text": "Try to persuade", "consequences": {"relationship_change": 1}},
                "intimidate": {"text": "Use intimidation", "consequences": {"relationship_change": -1, "fear_gained": True}},
                "negotiate": {"text": "Negotiate a deal", "consequences": {"compromise_reached": True}}
            },
            "combat": {
                "attack": {"text": "Attack immediately", "consequences": {"combat_initiated": True}},
                "defend": {"text": "Take defensive position", "consequences": {"defense_bonus": True}},
                "flee": {"text": "Attempt to escape", "consequences": {"combat_avoided": True, "reputation_change": -1}}
            }
        }
    ...
    def create_story_arc(self, title: str, description: str, arc_type: str = "main", 
                        themes: str = "", character_ids: str = "") -> Dict[str, Any]:
        """
    ...
    def generate_choices(self, situation: str, context: str = "", choice_type: str = "general",
                        num_choices: int = 3) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            return {
                "status": "success",
                "choices": [{"id": c.id, "text": c.text, "description": c.description} for c in choices],
                "situation": situation,
                "context": context
            }
        except Exception as e:
    ...
    def process_choice(self, choice_id: str, campaign_id: str, character_id: str = "",
        ...
        try:
            ...
            if campaign_id not in self.narrative_states:
                self.narrative_states[campaign_id] = NarrativeState(campaign_id=campaign_id)
            
            narrative_state = self.narrative_states[campaign_id]
            
            # Process consequences
            consequences = choice.consequences.copy()
            narrative_outcomes = self._apply_choice_consequences(choice, narrative_state, character_id)
    ...
    def advance_narrative(self, campaign_id: str, current_situation: str = "",
                         trigger_data: str = "") -> Dict[str, Any]:
        """
        ...
        try:
            ...
            for arc_id in narrative_state.active_story_arcs:
                if arc_id in self.story_arcs:
                    ...
                    arc_results = self._check_plot_point_triggers(arc, triggers, narrative_state)
            ...
            if activated_points or current_situation:
                ...
                if choice_result["status"] == "success":
                    ...
                    narrative_state.pending_choices.extend([c["id"] for c in new_choices])
    ...
    def _apply_choice_consequences(self, choice: NarrativeChoice, narrative_state: NarrativeState,
        ...
        for consequence_type, value in choice.consequences.items():
            if consequence_type == "relationship_change":
                ...
                # Update world state
                narrative_state.world_state.update(value if isinstance(value, dict) else {})
                outcomes["world_updated"] = True
        
        return outcomes

    def _check_plot_point_triggers(self, story_arc: StoryArc, triggers: Dict[str, Any],
                                 narrative_state: NarrativeState) -> Dict[str, List]:
        ...
        for plot_point_id in story_arc.plot_points:
            if plot_point_id in self.plot_points:
                ...
                if plot_point.status == "pending" and self._check_triggers(plot_point.triggers, triggers):
                ...
                elif plot_point.status == "active" and self._check_completion_criteria(plot_point, narrative_state):
    ...
    def _check_triggers(self, plot_triggers: Dict[str, Any], current_triggers: Dict[str, Any]) -> bool:
