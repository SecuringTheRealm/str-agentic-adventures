...
class CampaignService:
    ...
    def create_campaign(self, campaign_data: CreateCampaignRequest, is_custom: bool = True) -> Campaign:
    ...
    def list_campaigns(self, include_templates: bool = True, include_custom: bool = True) -> List[Campaign]:
    ...
    def get_templates(self) -> List[Campaign]:
        ...
        with next(get_session()) as db:
            db_campaigns = db.query(CampaignDB).filter(CampaignDB.is_template == True).all()
    ...
    def clone_campaign(self, template_id: str, new_name: Optional[str] = None) -> Optional[Campaign]:
        ...
        cloned_campaign = Campaign(
            ...
            tone=template.tone,
            homebrew_rules=template.homebrew_rules.copy() if template.homebrew_rules else [],
    ...
    def delete_campaign(self, campaign_id: str) -> bool:
        ...
        with next(get_session()) as db:
            db_campaign = db.query(CampaignDB).filter(
                CampaignDB.id == campaign_id,
                CampaignDB.is_template == False  # Only allow deletion of non-templates
            ).first()
