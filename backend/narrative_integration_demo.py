        ...
        try:
            ...
            if main_arc_result["status"] == "success":
                ...
                self.narrative_memory.record_event(
                    f"Campaign '{request.name}' created", request.setting, "", 8
                )
    ...
    def process_player_input(self, player_input: PlayerInput) -> GameResponse:
        ...
        try:
            ...
            if any(word in message.lower() for word in ["fight", "attack", "combat"]):
                ...
                word in message.lower() for word in ["explore", "investigate", "search"]
            ):
                choice_type = "exploration"

            # Generate narrative choices
            choices_result = self.narrative_gen.generate_choices(
                situation=message, choice_type=choice_type, num_choices=3
            )
            ...
            if advance_result.get("activated_plot_points"):
                response_message += (
                    " Your actions have triggered significant story developments!"
                )
            ...
            state_updates = {
                ...
                "plot_points_activated": advance_result.get(
                    "activated_plot_points", []
                ),
            ...
            return GameResponse(message=response_message, state_updates=state_updates)
    ...
    def get_campaign_narrative_status(self, campaign_id: str) -> dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "summary": {
                    "active_arcs": len(narrative_state.get("active_story_arcs", []))
                    if narrative_state.get("status") == "success"
                    else 0,
                    "pending_choices": len(narrative_state.get("pending_choices", []))
                    if narrative_state.get("status") == "success"
                    else 0,
    ...
    def make_narrative_choice(
        ...
        try:
            ...
            process_result = self.narrative_gen.process_choice(
                choice_id=choice_id, campaign_id=campaign_id, character_id=character_id
            )
