...
def check_basic_srd_compliance():
    ...
    core_elements = {
        ...
        'combat_actions': ['attack', 'dodge', 'dash', 'disengage', 'help', 'hide', 'ready', 'search'],
    ...
    for category, data in results.items():
        status = "🟢" if data['percentage'] >= 80 else "🟡" if data['percentage'] >= 50 else "🔴"
        report.append(f"### {status} {category.upper()}: {data['percentage']:.1f}% ({data['implemented']}/{data['total']})")
    ...
    if results['classes']['percentage'] < 100:
        ...
        report.append("- Implement missing character class support in character creation")
...
def main():
    ...
    # Save report
    report_file = Path("/home/runner/work/str-agentic-adventures/str-agentic-adventures/docs/srd_compliance_report.md")
