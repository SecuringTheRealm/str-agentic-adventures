...
class SRDRulesChecker:
    ...
    def parse_srd(self):
        ...
        print(f"✅ Parsed SRD: Found {sum(len(rules) for rules in self.rules_found.values())} rule elements")
    ...
    def _extract_spells(self, content: str):
        ...
        spell_entries = re.findall(r'\n([A-Z][a-zA-Z\s\']+)\n.*?(?:1st-level|2nd-level|3rd-level|4th-level|5th-level|6th-level|7th-level|8th-level|9th-level|cantrip)', content, re.MULTILINE)
        
        all_spells = set(spells + spell_entries)
        
        # Filter common words that aren't spells
        common_words = {'The', 'A', 'An', 'And', 'Or', 'But', 'In', 'On', 'At', 'To', 'For', 'Of', 'With', 'By'}
        spell_list = [spell.strip() for spell in all_spells if spell.strip() not in common_words and len(spell.strip()) > 2]
    ...
    def _extract_classes(self, content: str):
        ...
        known_classes = ['Barbarian', 'Bard', 'Cleric', 'Druid', 'Fighter', 'Monk', 'Paladin', 'Ranger', 'Rogue', 'Sorcerer', 'Warlock', 'Wizard']
    ...
    def _extract_races(self, content: str):
        ...
        race_pattern = r'### ([A-Z][a-z]+(?:\s+[A-Z][a-z]+)*)\s*\n.*?(?:size|speed|ability)'
        races = re.findall(race_pattern, content, re.MULTILINE | re.DOTALL | re.IGNORECASE)
        
        # Known D&D 5e races
        known_races = ['Human', 'Elf', 'Dwarf', 'Halfling', 'Dragonborn', 'Gnome', 'Half-Elf', 'Half-Orc', 'Tiefling']
        
        found_races = [race for race in races if any(known_race in race for known_race in known_races)]
    ...
    def _check_spell_compliance(self) -> Dict[str, any]:
        ...
        spell_files = list(self.repo_root.glob("**/*spell*.py")) + list(self.repo_root.glob("**/*magic*.py"))
        ...
        return {
            ...
            'percentage': (implemented_count / total_spells * 100) if total_spells > 0 else 0,
            'missing': list(set(self.rules_found['spells']) - implemented_spells)[:10]  # Show first 10 missing
        }
    
    def _check_class_compliance(self) -> Dict[str, any]:
        """Check if character classes are implemented."""
        # Look for class implementations
        class_files = list(self.repo_root.glob("**/*class*.py")) + list(self.repo_root.glob("**/*character*.py"))
        ...
        return {
            'total': total_classes,
            'implemented': implemented_count,
            'percentage': (implemented_count / total_classes * 100) if total_classes > 0 else 0,
    ...
    def _check_race_compliance(self) -> Dict[str, any]:
        ...
        race_files = list(self.repo_root.glob("**/*race*.py")) + list(self.repo_root.glob("**/*character*.py"))
        ...
        return {
            'total': total_races,
            'implemented': implemented_count,
            'percentage': (implemented_count / total_races * 100) if total_races > 0 else 0,
    ...
    def _check_equipment_compliance(self) -> Dict[str, any]:
        ...
        equipment_files = list(self.repo_root.glob("**/*equipment*.py")) + \
        ...
        return {
            ...
            'implemented': implemented_count,
            'percentage': (implemented_count / total_equipment * 100) if total_equipment > 0 else 0,
            'missing': list(set(self.rules_found['equipment']) - implemented_equipment)[:10]
        }
    
    def _check_combat_compliance(self) -> Dict[str, any]:
        """Check if combat rules are implemented."""
        combat_files = list(self.repo_root.glob("**/*combat*.py")) + \
        ...
        return {
            'total': total_rules,
            'implemented': implemented_count,
            'percentage': (implemented_count / total_rules * 100) if total_rules > 0 else 0,
    ...
    def generate_compliance_report(self, compliance_data: Dict) -> str:
        ...
        total_percentage = sum(category['percentage'] for category in compliance_data.values()) / len(compliance_data)
        report.append(f"## Overall Compliance: {total_percentage:.1f}%")
        report.append("")
        
        for category, data in compliance_data.items():
            status_icon = "🟢" if data['percentage'] >= 80 else "🟡" if data['percentage'] >= 50 else "🔴"
            report.append(f"### {status_icon} {category.upper()} - {data['percentage']:.1f}% Implemented")
            ...
            if data['missing']:
                report.append(f"- **Examples of Missing**: {', '.join(data['missing'][:5])}")
            report.append("")
