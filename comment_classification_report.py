...
class CommentClassifier:
        ...
        try:
            ...
            for line_num, line in enumerate(lines, 1):
                ...
                if comment_text:
                    ...
                    comments.append((str(file_path.relative_to(self.repo_root)), 
                                   line_num, comment_text.strip(), classification))
    ...
    def classify_comment(self, comment: str) -> str:
        ...
        doc_keywords = ['param', 'return', 'args', 'type', 'interface', 'class', 'function', 'method']
    ...
    def scan_repository(self):
        ...
        exclude_dirs = {'.git', 'node_modules', '__pycache__', '.pytest_cache', 'build', 'dist'}
    ...
    def generate_report(self) -> str:
        ...
        report.append(f"- **Documentation Comments**: {len(self.results['documentation'])}")
        ...
        if self.results['documentation']:
            doc_files = set(comment['file'] for comment in self.results['documentation'])
            report.append(f"Found documentation comments in {len(doc_files)} files:")
            for file_path in sorted(doc_files):
                count = len([c for c in self.results['documentation'] if c['file'] == file_path])
        ...
        if self.results['regular']:
            ...
            for file_path in sorted(regular_files):
                count = len([c for c in self.results['regular'] if c['file'] == file_path])
